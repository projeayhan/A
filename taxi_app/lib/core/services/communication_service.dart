...
/// Güvenli İletişim Servisi
/// Sürücü-Müşteri arasındaki tüm iletişimi yönetir
class CommunicationService {
  static SupabaseClient get _client => SupabaseService.client;

  // ==================== SECURE CONTACT INFO ====================

  /// Güvenli müşteri bilgisi getir (sürücü için - maskelenmiş)
  static Future<SecureCustomerInfo?> getSecureCustomerInfo(String rideId) async {
    try {
      final response = await _client.rpc('get_secure_customer_info', params: {
        'p_ride_id': rideId,
        'p_driver_user_id': SupabaseService.currentUser?.id,
      });

      if (response != null && response is List && response.isNotEmpty) {
        return SecureCustomerInfo.fromJson(response[0]);
      }
      return null;
    } catch (e, st) {
      LogService.error('getSecureCustomerInfo error', error: e, stackTrace: st, source: 'CommunicationService:getSecureCustomerInfo');
      return null;
    }
  }
  ...
  static Future<bool> updateCallStatus({
    ...
    try {
      ...
      LogService.error('updateCallStatus error', error: e, stackTrace: st, source: 'CommunicationService:updateCallStatus');
      return false;
    }
  }
  ...
  static Future<List<QuickMessage>> getQuickMessages({
    required String userType, // 'driver' veya 'customer'
  }) async {
    try {
      final response = await _client
          .from('quick_messages')
  ...
  static void _startSosLocationStreaming() {
    ...
    _sosLocationTimer = Timer.periodic(const Duration(seconds: 10), (_) async {
      try {
        ...
        LogService.error('SOS location stream error', error: e, stackTrace: st, source: 'CommunicationService:startSosTracking');
      }
    });
  }
  ...
  static Future<void> sendSosToEmergencyContacts({
    ...
    for (final contact in contacts) {
      ...
      try {
        ...
        LogService.error('SOS send error for ${contact.name}', error: e, stackTrace: st, source: 'CommunicationService:_sendSosMessages');
      }
    }
    ...
    try {
      ...
      LogService.error('155 arama hatası', error: e, stackTrace: st, source: 'CommunicationService:call155');
    }
  }
}
...
/// Güvenli müşteri bilgisi (sürücü görür)
class SecureCustomerInfo {
  ...
  factory SecureCustomerInfo.fromJson(Map<String, dynamic> json) {
    return SecureCustomerInfo(
