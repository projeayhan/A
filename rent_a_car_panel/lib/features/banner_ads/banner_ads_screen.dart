...
final myBannersProvider = FutureProvider<List<Map<String, dynamic>>>((
  ...
  final companyOwnerId = _supabase.auth.currentUser?.id;
  if (companyOwnerId == null) return [];
  // rent_a_car uses company owner as merchant_id for banners
  final data = await _supabase
      .from('banners')
      .select('*, banner_packages(name, duration_days)')
      .eq('merchant_id', companyOwnerId)
