...
class _TicketReviewScreenState extends ConsumerState<TicketReviewScreen> {
  ...
  Future<void> _loadTicketDetail(String ticketId) async {
    setState(() { _selectedTicketId = ticketId; _loadingDetail = true; });
    try {
      final service = ref.read(supportMonitoringServiceProvider);
      final results = await Future.wait([
      ...
      setState(() {
        ...
        _selectedMessages = results[0] as List<Map<String, dynamic>>;
        _selectedActions = results[1] as List<Map<String, dynamic>>;
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Text('Ticket Inceleme', style: TextStyle(color: textPrimary, fontSize: 24, fontWeight: FontWeight.bold)),
                          const Spacer(),
                          Text('$_totalCount ticket', style: TextStyle(color: textMuted, fontSize: 13)),
                          const SizedBox(width: 12),
                          IconButton(
                            onPressed: _loadData,
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _buildDropdown(cardBg, borderColor, 'Durum', _statusFilter, {
                            '': 'Tumunu',
                            'open': 'Acik',
                            'assigned': 'Atanmis',
                            'in_progress': 'Islemde',
                            ...
                            'resolved': 'Cozuldu',
                            'closed': 'Kapandi',
                          }, (v) { setState(() { _statusFilter = v ?? ''; _page = 0; }); _loadData(); }),
                          _buildDropdown(cardBg, borderColor, 'Oncelik', _priorityFilter, {
                            '': 'Tumunu',
                            'low': 'Dusuk',
                            'normal': 'Normal',
                            'high': 'Yuksek',
                            'urgent': 'Acil',
                          }, (v) { setState(() { _priorityFilter = v ?? ''; _page = 0; }); _loadData(); }),
                          _buildDropdown(cardBg, borderColor, 'Servis', _serviceFilter, {
                            '': 'Tumunu',
                            'food': 'Yemek',
                            'taxi': 'Taksi',
                            'rental': 'Kiralama',
                            ...
                            'general': 'Genel',
                          }, (v) { setState(() { _serviceFilter = v ?? ''; _page = 0; }); _loadData(); }),
                          if (_agents.isNotEmpty)
                            _buildDropdown(cardBg, borderColor, 'Temsilci', _agentFilter, {
                              '': 'Tumunu',
                              ..._agents.fold<Map<String, String>>({}, (map, a) {
                                map[a['id']] = a['full_name'] ?? '';
                                return map;
                              }),
                            }, (v) { setState(() { _agentFilter = v ?? ''; _page = 0; }); _loadData(); }),
                          if (_statusFilter.isNotEmpty || _priorityFilter.isNotEmpty || _serviceFilter.isNotEmpty || _agentFilter.isNotEmpty || _searchQuery.isNotEmpty)
                            TextButton.icon(
                ...
                Expanded(
                  child: _loading
                      ...
                      : _error != null
                          ...
                          : _tickets.isEmpty
                              ? Center(child: Text('Ticket bulunamadi', style: TextStyle(color: textMuted)))
                              : ListView.builder(
                                  itemCount: _tickets.length,
                                  itemBuilder: (context, index) {
                                    final ticket = _tickets[index];
                                    final isSelected = _selectedTicketId == ticket['id'];
                                    return _buildTicketRow(ticket, isSelected, cardBg, borderColor, textPrimary, textMuted, isDark);
                                  },
                                ),
                ),
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text('${_page + 1} / ${(_totalCount / _pageSize).ceil()}', style: TextStyle(color: textPrimary)),
                        IconButton(
                          onPressed: (_page + 1) * _pageSize < _totalCount ? () { setState(() => _page++); _loadData(); } : null,
  ...
  Widget _buildDropdown(Color bg, Color border, String hint, String value, Map<String, String> items, ValueChanged<String?> onChanged) {
    return Container(
  ...
  Widget _buildTicketRow(Map<String, dynamic> ticket, bool isSelected, Color cardBg, Color borderColor, Color textPrimary, Color textMuted, bool isDark) {
          ...
          child: Row(
            children: [
              ...
              Container(
                width: 40, height: 40,
                decoration: BoxDecoration(
                  color: SupportMonitoringService.serviceColor(serviceType).withValues(alpha: 0.12),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Icon(SupportMonitoringService.serviceIcon(serviceType),
                    color: SupportMonitoringService.serviceColor(serviceType), size: 20),
              ),
              ...
              Expanded(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('#$ticketNumber', style: TextStyle(color: textMuted, fontSize: 12, fontWeight: FontWeight.w600)),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(ticket['subject'] ?? '-', style: TextStyle(color: textPrimary, fontSize: 13, fontWeight: FontWeight.w500),
                              maxLines: 1, overflow: TextOverflow.ellipsis),
                        ),
                      ],
                    ),
                    const SizedBox(height: 4),
                    Text(ticket['customer_name'] ?? 'Anonim', style: TextStyle(color: textMuted, fontSize: 12)),
                  ],
                ),
              ),
              const SizedBox(width: 12),
              ...
              const SizedBox(width: 8),
              // Priority
              _buildBadge(SupportMonitoringService.priorityLabel(priority), SupportMonitoringService.priorityColor(priority)),
              const SizedBox(width: 12),
              // Agent
              SizedBox(
                width: 100,
                child: Text(agentName ?? 'Atanmamis', style: TextStyle(color: agentName != null ? textPrimary : textMuted, fontSize: 12),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
              ),
              const SizedBox(width: 12),
              // Date
  ...
  Widget _buildDetailPanel(Color cardBg, Color borderColor, Color textPrimary, Color textMuted, bool isDark) {
    ...
    if (_selectedTicket == null) {
      return Center(child: Text('Ticket secin', style: TextStyle(color: textMuted)));
    }
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('#${ticket['ticket_number']}', style: TextStyle(color: AppColors.primary, fontSize: 16, fontWeight: FontWeight.bold)),
                        const SizedBox(width: 8),
                        _buildBadge(SupportMonitoringService.statusLabel(status), SupportMonitoringService.statusColor(status)),
                        const SizedBox(width: 6),
                        _buildBadge(SupportMonitoringService.priorityLabel(priority), SupportMonitoringService.priorityColor(priority)),
                        const SizedBox(width: 6),
                        _buildBadge(SupportMonitoringService.serviceLabel(serviceType), SupportMonitoringService.serviceColor(serviceType)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Text(ticket['subject'] ?? '-', style: TextStyle(color: textPrimary, fontSize: 14, fontWeight: FontWeight.w500)),
                    const SizedBox(height: 4),
                    Text('Musteri: ${ticket['customer_name'] ?? 'Anonim'} | ${ticket['customer_phone'] ?? '-'}',
                        style: TextStyle(color: textMuted, fontSize: 12)),
                  ],
                ),
              ),
              IconButton(
                onPressed: () => setState(() { _selectedTicketId = null; _selectedTicket = null; }),
        ...
        Expanded(
          child: _selectedMessages == null || _selectedMessages!.isEmpty
              ? Center(child: Text('Mesaj yok', style: TextStyle(color: textMuted)))
              : ListView.builder(
                  padding: const EdgeInsets.all(16),
                  itemCount: _selectedMessages!.length,
                  itemBuilder: (context, index) {
                    final msg = _selectedMessages![index];
                    return _buildMessageBubble(msg, textPrimary, textMuted, isDark);
                  },
                ),
        ),
        ...
        if (_selectedActions != null && _selectedActions!.isNotEmpty)
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Aksiyon Logu', style: TextStyle(color: textPrimary, fontSize: 12, fontWeight: FontWeight.w600)),
                const SizedBox(height: 6),
                Expanded(
                      ...
                      return Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            Text(action['agent_name'] ?? '', style: TextStyle(color: AppColors.primary, fontSize: 11, fontWeight: FontWeight.w500)),
                            const SizedBox(width: 6),
                            Expanded(
                              child: Text(action['action_description'] ?? action['action_type'] ?? '',
                                  style: TextStyle(color: textMuted, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis),
                            ),
                          ],
                        ),
                      );
  ...
  Widget _buildMessageBubble(Map<String, dynamic> msg, Color textPrimary, Color textMuted, bool isDark) {
      ...
      return Padding(
        ...
        child: Center(
          child: Container(
            ...
            child: Text(msg['message'] ?? '', style: TextStyle(color: textMuted, fontSize: 11, fontStyle: FontStyle.italic)),
          ),
        ),
      );
    }
      ...
      child: Align(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(12),
            border: isWhisper ? Border.all(color: const Color(0xFFF59E0B).withValues(alpha: 0.3)) : null,
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
                ...
                children: [
                  Text(
                    isWhisper ? '(Fisildama) ${msg['sender_name'] ?? ''}' : msg['sender_name'] ?? (isAgent ? 'Temsilci' : 'Musteri'),
                    style: TextStyle(
                      color: isWhisper ? const Color(0xFFF59E0B) : AppColors.primary,
                      fontSize: 11,
                  ...
                  if (createdAt != null) ...[
                    const SizedBox(width: 8),
                    Text(DateFormat('HH:mm').format(createdAt.toLocal()), style: TextStyle(color: textMuted, fontSize: 10)),
                  ],
                ],
              ),
              const SizedBox(height: 4),
              Text(msg['message'] ?? '', style: TextStyle(color: textPrimary, fontSize: 13)),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('Ek dosya', style: TextStyle(color: AppColors.info, fontSize: 11)),
                    ],
                  ),
                ),
            ],
