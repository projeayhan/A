...
class LogService {
  static const String appName = 'admin_panel';
  ...
  static void _enqueue(
    ...
    // In release mode, only send error and warn
    ...
    // Flush if queue reaches 10 items
    if (_queue.length >= 10) {
      _flush();
    } else {
      _flushTimer?.cancel();
  ...
  /// Force flush remaining logs (call on app dispose/pause)
