  ...
  void initState() {
    ...
    _moveController.addStatusListener((status) {
      if (!_isManualPosition && !_isChatOpen) {
        final nowRight = status == AnimationStatus.forward;
        if (nowRight != _movingRight) {
          setState(() => _movingRight = nowRight);
        }
      }
    });
  ...
  Future<void> _startVoiceMode() async {
    ...
    if (!initialized) {
      setState(() {
        ...
        _voiceResponseText =
            'Ses tanima baslatilamadi. Mikrofon izni verdiginizden emin olun.';
    ...
    if (!started && mounted) {
      setState(() {
        _isVoiceMode = false;
        _voiceResponseText =
            'Ses dinleme baslatilamadi: ${voiceInputService.lastError}';
        _showVoiceResponse = true;
      });
      _voiceResponseDismissTimer?.cancel();
      _voiceResponseDismissTimer = Timer(const Duration(seconds: 5), () {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        if (inlineAudio != null && inlineAudio.isNotEmpty) {
          voiceOutputService.playBase64Audio(
            inlineAudio,
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: const Color(0xFF00FF88), blurRadius: 6),
                    ],
                  ),
                ),
    ...
    if (existingSessionId != null) {
      ...
      _messages.add(
        _ChatMessage(
          role: 'assistant',
          content:
              'Merhaba! Ben admin AI asistani. Platform yonetimi, istatistikler, kullanici/isletme/siparis islemleri ile ilgili sorularinizi yanitlayabilirim.',
          ...
          case AiStreamEventType.error:
            setState(() {
              ...
              if (assistantMessage.content.isEmpty) {
                assistantMessage.content = 'Uzgunum, ${event.error}';
              }
            });
            _scrollToBottom();
            break;
      ...
      onDone: () {
        if (mounted && assistantMessage.isStreaming) {
          setState(() {
            ...
            _isLoading = false;
            if (assistantMessage.content.isEmpty) {
              assistantMessage.content = 'Baglanti kesildi.';
            }
          });
          _scrollToBottom();
        }
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Komut veya soru yazin...',
                    hintStyle: TextStyle(
