...
class _CannedResponsesScreenState extends ConsumerState<CannedResponsesScreen> {
  ...
  Future<void> _loadResponses() async {
    ...
    try {
      ...
      LogService.error('Failed to load canned responses', error: e, stackTrace: st, source: 'CannedResponsesScreen:_loadResponses');
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              if (r['shortcut'] != null &&
                                  (r['shortcut'] as String).isNotEmpty)
                                Container(
                                  padding: const EdgeInsets.symmetric(
                                    horizontal: 8,
                                    vertical: 2,
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: Text(
            existing == null ? 'Yeni Hazir Yanit' : 'Hazir Yaniti Duzenle',
          ),
          content: SizedBox(
            width: 500,
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Kisayol (opsiyonel)',
                    border: OutlineInputBorder(),
                    hintText: 'ornegin: selamlama',
                  ),
                ),
                const SizedBox(height: 12),
  ...
  Future<void> _deleteResponse(String id) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Bu hazir yaniti silmek istediginizden emin misiniz?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
