...
final _ticketListProvider =
    StateNotifierProvider<_TicketListNotifier, _TicketListState>((ref) {
...
class _TicketListNotifier extends StateNotifier<_TicketListState> {
  ...
  void _subscribeRealtime() {
    ...
    _channel = supabase
        .channel('tickets_screen_realtime')
        .onPostgresChanges(
  ...
  Future<void> fetch() async {
    ...
    try {
      ...
      LogService.error('Failed to load tickets', error: e, stackTrace: st, source: 'TicketsNotifier:loadTickets');
  ...
  void setStatusFilter(String? v) {
    state = state.copyWith(statusFilter: v, page: 0);
    fetch();
  }

  void setPriorityFilter(String? v) {
    state = state.copyWith(priorityFilter: v, page: 0);
    fetch();
  }

  void setServiceTypeFilter(String? v) {
    state = state.copyWith(serviceTypeFilter: v, page: 0);
    fetch();
  }

  void toggleMyTickets() {
    state = state.copyWith(myTicketsOnly: !state.myTicketsOnly, page: 0);
    fetch();
  }

  void setSort(String col, bool asc) {
    state = state.copyWith(sortColumn: col, sortAscending: asc);
    fetch();
  }
}
            ...
            children: [
              ...
              FilterChip(
                ...
                labelStyle: TextStyle(
                  color: s.myTicketsOnly ? AppColors.primary : textMuted,
                  fontSize: 12,
                ),
                side: BorderSide(
                  color: s.myTicketsOnly ? AppColors.primary : borderColor,
                ),
              ),
