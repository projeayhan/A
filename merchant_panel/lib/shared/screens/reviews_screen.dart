...
class _ReviewsScreenState extends ConsumerState<ReviewsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // Stats Header
        _buildStatsHeader(reviewsAsync, isDark),
  ...
  Widget _buildStatsHeader(AsyncValue<List<Map<String, dynamic>>> reviewsAsync, bool isDark) {
    return Container(
      ...
      child: reviewsAsync.when(
        data: (reviews) {
          ...
          final answeredCount = reviews.where((r) => r['merchant_reply'] != null).length;
          final responseRate = totalReviews > 0
              ? ((answeredCount / totalReviews) * 100).toStringAsFixed(0)
              : '0';
                  ...
                  Expanded(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        // Check for profanity
                        if (ProfanityFilter.containsProfanity(replyText)) {
                          AppDialogs.showError(context, 'Yanıtınız uygunsuz ifadeler içeriyor. Lütfen düzenleyin.');
                          return;
                        }

                        await _submitReply(review['id'] as String, replyText);
                        if (context.mounted) {
